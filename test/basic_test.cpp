...
public:
  ...
  void testHelloWorld() {
    CompletedProcess completed = subprocess::run(
  ...
  void testHelloWorldBuilder() {
    CompletedProcess completed =
      subprocess::RunBuilder({"echo", "hello", "world"})
...
// this experimental value.
  ...
  void test2ProcessConnect() {
    ...
    subprocess::Popen cat =
      RunBuilder({"cat"}).cout(PipeOption::pipe).cin(pipe.input).popen();
    subprocess::Popen echo =
      RunBuilder({"echo", "hello", "world"}).cout(pipe.output).popen();
    pipe.close();
